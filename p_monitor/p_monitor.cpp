...
double dynamixel_command(String cmd[])
{
...

	{
		...
		else if (isAvailableID(cmd[1].toInt()))
		{
			...
			//****************************************************
			else if (cmd[0] == "ReadActPos")
			{
				uint8_t id = cmd[1].toInt();
				int32_t datatest = 0;

				result = dxl_wb.getPresentPositionData(id, &datatest, &log);
				if (result == false)
				{
					Serial.println(log);
					return 0;
			...
			//****************************************************
			//****************************************************
			else if (cmd[0] == "ReadMaxPos")
			{
				uint8_t id = cmd[1].toInt();
				int32_t data = 0;

				result = dxl_wb.itemRead(id, "Max_Position_Limit", &data, &log);
				if (result == false)
				{
					Serial.println(log);
					return 0;
			...
			//****************************************************
			//****************************************************
			else if (cmd[0] == "writetest")
			{
				uint8_t id = cmd[1].toInt();
				int32_t data = cmd[3].toInt();

				result = dxl_wb.torqueOff(id, &log);

				result = dxl_wb.itemWrite(id, cmd[2].c_str(), data, &log);
				if (result == false)
				{
					Serial.println(log);
					return 0;
			...
			//****************************************************
			//****************************************************
			else if (cmd[0] == "ReadCurrent")
			{
				uint8_t id = cmd[1].toInt();
				int32_t data = 0;

				result = dxl_wb.itemRead(id, "Present_Current", &data, &log);
				if (result == false)
				{
					Serial.println(log);
					return 0;
				...
				{
					//Serial.println(log);
					//Serial.println(dxl_wb.convertValue2Current(data));
					//Serial.println(dxl_wb.convertValue2Load(data));
					//Serial.println(data);
					return double(dxl_wb.convertValue2Current(data));
				}
			}
			//****************************************************
			else if (cmd[0] == "wheel")
